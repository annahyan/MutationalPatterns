...
new MutationalPatterns article. TODO: Write article. The old article can be
...
However, this is only usefull if you have a large number of mutations.
...
In these cases it might be usefull to combine multiple samples.
...
NMF is most usefull for large amounts of samples, 
...
signatures that are already known. Therefore, it might be usefull to change the
...
will use signatures from [COSMIC](https://cancer.sanger.ac.uk/cosmic/signatures)
(v3.1) [@Alexandrov2020]. (We will discuss how to use other signature matrixes
later.)
```{r read_signatures}
...
non-negative least-squares constraints problem.
...
subjective. Here we use a cutoff of 0.02. Decreasing this number will make the
refitting less strict, while increasing it will make the refitting more strict.
...
Some signatures are more likely than others to have functional effects, by causing "stop gain" or "mismatch" mutations.
With MutationalPatterns it's possible to analyze how likely it is for a signature to either cause "stop gain", "mismatch"
or "synonymous" mutations for a set of genes of interest.

First you need to load a transcription annotation database.
...
Now the ratio of "stop gain", "mismatch" and "synonymous" mutations can be
...
using them for most analyses. However, these signatures can be usefull to see if
...
Using an incorrect `tissue_type` will result in an error. This is usefull,
...
see the Biocpkg("biomaRt") documentation [@Durinck2005]. 
...
You can now plot this using `plot_long_profile`. The arguments of
...
```{r plot_long_profile, fig.wide=TRUE}
...
be usefull to, for example, compare the spectrum of regions with kataegis with
...
You can visualize possible lesion segregation for a single sample. If lesion
segregation is present, then it will generally be quite clear that the mutations
are not randomly distributed over the strands. In this example no lesion
segregation is present. ("+" and "-" are used instead of "Watson" and "Crick" to
save space.)
```{r plot_lesion_segregation}
plot_lesion_segregation(grl[[1]])
```
...
based on how often two suqsequent mutations occur on the same strand. The
...
strand assymetries like replication-associated strand bias. As a result the
p-value can incorrectly suggest that lesion segregation is present. Therefore,
it can be usefull to also look at the fraction of strand switches. In samples
...
Therefore, it might be usefull to calculate the number of strand switches per
...
The second mode of `calculate_lesion_segregation` uses the Walf-Wolfowitz test,
...
the first mode.
```{r calculate_lesion_segregation_walf}
lesion_segretation_walf <- calculate_lesion_segregation(grl, sample_names, 
                                                        test = "walf-wolfowitz")
head(lesion_segretation_walf)
```

This statistical test can also be influenced by events such as kataegis and 
local strand assymetries like replication-associated strand bias. 
